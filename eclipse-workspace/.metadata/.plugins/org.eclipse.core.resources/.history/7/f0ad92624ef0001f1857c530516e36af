...
public class Bbdd_Control {
        ...
        try {
            ...
            if (m.getIdMascotas() != -1) {
                registro = consulta.executeQuery("select idMascota, nombre, tipoAnimal, edad, descripcionSintomas, vacunas "
                        ...
                        + m.getIdMascotas());
            } else if (!m.getNombre().equals("ERROR")) { // Preguntamos por la descripción
                registro = consulta.executeQuery("select idMascota, nombre, tipoAnimal, edad, descripcionSintomas, vacunas "
    ...
    public void muestraMascota(Mascotas miMascota) {
        ...
        System.out.println("Descripción de los síntomas: " + miMascota.getDescripcionSintomas());
    ...
    public void modificarDatos() {
        ...
        try {
            ...
            if (valor == 1) {
                c.muestraCadena("Artículo modificado correctamente");
            } else {
                c.muestraCadena("No existe un artículo con dicho identificador");
            }
            conexion.close();
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public void borraDatos() {
        ...
        try {
            ...
            int valor = consulta.executeUpdate("delete from mascotas where idMascota="
                    + idMascota);
            if (valor == 1) {
                c.muestraCadena("Artí­culo borrado correctamente");
            } else {
    ...
    public void modificarDatosConPrepareStatement() {
        ...
        try {
            ...
            if(rs==1){
                c.muestraCadena(nombreAnimal+" modificado con éxito.");
            }else{
